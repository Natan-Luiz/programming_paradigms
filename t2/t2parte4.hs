...
shiftChar c i = if isLower c then decodeChar (encoded c i) else c
  where encoded c i = mod (encodeChar c + i) 26

--Q2
encodeStr :: String -> Int -> String
encodeStr str i = map (\x -> shiftChar x i) str
...
--Q5
freqs :: String -> [Float]
freqs str = [((fromIntegral x)/(fromIntegral (countValids str)))*100 | x <- lst]
  where lst = map(\y -> countChar y str ) ['a'..'z']

--Q6
positions :: Float -> [Float] -> [Int]
positions f lst = filter (>=0) (map(\x -> if f == fst x then snd x else -1) (zip lst [0,1..]))
